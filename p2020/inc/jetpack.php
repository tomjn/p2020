...
add_action( 'after_setup_theme', 'breathe_infinite_scroll_setup' );

/**
 * Switch to 'click' method if is_mobile and sidebar-1 is active
 */
if ( function_exists( 'jetpack_is_mobile' ) ) {
    function breathe_has_footer_widgets() {
        if ( jetpack_is_mobile( '', true ) && is_active_sidebar( 'sidebar-1' ) )
            return true;

        return false;
    }
    add_filter( 'infinite_scroll_has_footer_widgets', 'breathe_has_footer_widgets' );
}

/**
 * Add theme support for o2.
 */
function breathe_o2_setup() {
